...
void Vocabulary::load_from_yaml_text(const std::string &filename)
{
  ...
  while (std::getline(file, line))
  {
    ...
    else if (line == "nodes:" || line.find("nodes:") != std::string::npos)
    {
  ...
  while (std::getline(file, line))
  {
    ...
    if (line == "words:" || line.find("words:") != std::string::npos)
    {
      if (reading_node)
      {
    ...
    if (current_section == "nodes")
    {
      if (line.find("- nodeId:") != std::string::npos)
      {
        if (reading_node)
        {
          ...
          {
            std::cout << "已解析 " << nodes_count << " 个节点..." << std::endl;
          }
        }
      ...
      else if (line.find("descriptor:") != std::string::npos && reading_node)
      {
        ...
        if (current_temp_node.descriptor.empty())
        {
          std::cout << "警告: 节点 " << current_temp_node.id << " 描述符解析失败" << std::endl;
        }
      }
    }
  }
  ...
  while (std::getline(file, line))
  {
    ...
    else if (line.find("nodeId:") != std::string::npos)
    {
      ...
      if (word_id >= 0 && node_id >= 0)
      {
        ...
        {
          std::cout << "已解析 " << words_count << " 个单词映射..." << std::endl;
        }
      }
    }
  }
...
cv::Mat Vocabulary::parse_descriptor_string(const std::string &desc_str)
{
  ...
  std::string trimmed = trim(desc_str);
  std::cout << "解析描述符: " << trimmed.substr(0, 50) << "..." << std::endl;

  // 🔍 **格式识别 - 按优先级检测**

  // 1. 检测空格分隔的二进制格式 (你的YAML格式)
  // "dbw3 0 256 1 1 0 1 0..."
  if (trimmed.find("dbw3") == 0)
  {
    std::cout << "检测到dbw3空格分隔二进制格式" << std::endl;
    return parse_space_separated_descriptor(trimmed);
  }

  // 2. 检测大量空格分隔的数字（可能是二进制位）
  if (trimmed.find(' ') != std::string::npos &&
      std::count(trimmed.begin(), trimmed.end(), ' ') > 10)
  {
    // 检查是否全是0和1
    ...
    if (all_binary && token_count > 10)
    {
      std::cout << "检测到空格分隔二进制格式" << std::endl;
      return parse_space_separated_descriptor(trimmed);
    }
  }
  ...
  {
    std::cout << "检测到方括号数组格式" << std::endl;
    return parse_float_array_descriptor(trimmed);
  }
  ...
  if (trimmed.find(',') != std::string::npos)
  {
    std::cout << "检测到逗号分隔格式" << std::endl;
    return parse_comma_separated_descriptor(trimmed);
  }
  ...
  if (trimmed.length() > 50 &&
      std::all_of(trimmed.begin(), trimmed.end(),
                  ...
                  { return c == '0' || c == '1'; }))
  {
    std::cout << "检测到纯二进制字符串格式" << std::endl;
    return parse_binary_string_descriptor(trimmed);
  }
  ...
  {
    std::cout << "检测到十六进制格式" << std::endl;
    return parse_hex_descriptor(trimmed);
  }

  // 7. 默认尝试空格分隔数值
  std::cout << "使用默认空格分隔数值解析" << std::endl;
  return parse_space_separated_numbers(trimmed);
}

// 🎯 **核心方法：解析空格分隔的描述符（你的YAML格式）**
cv::Mat Vocabulary::parse_space_separated_descriptor(const std::string &desc_str)
{
  if (desc_str.empty())
  {
    return cv::Mat();
  ...
  if (tokens.size() < 4)
  {
    std::cout << "警告: 描述符格式错误，token数量: " << tokens.size() << std::endl;
    return cv::Mat();
  }
  ...
  if (tokens[0] != "dbw3")
  {
    std::cout << "警告: 不是dbw3格式: " << tokens[0] << std::endl;
    // 尝试按通用方式解析
    return parse_generic_binary_descriptor(tokens);
  }

  // 🎯 **解析描述符长度 - 正确位置是tokens[2]**
  int descriptor_bits = 256; // 默认值
  try
  {
    if (tokens.size() > 2)
    {
      descriptor_bits = std::stoi(tokens[2]); // tokens[2] 是 "256"
    }
  }
  catch (const std::exception &e)
  {
    std::cout << "警告: 无法解析描述符长度 '" << tokens[2] << "', 使用默认值256" << std::endl;
  }

  // 🎯 **提取二进制位数据 (从tokens[3]开始)**
  std::vector<int> binary_bits;
  for (size_t i = 3; i < tokens.size(); ++i)
  {
    try
    {
      int bit = std::stoi(tokens[i]);
      if (bit == 0 || bit == 1)
      {
      ...
      {
        std::cout << "警告: 无效位值 " << bit << " 在位置 " << i << std::endl;
      }
    }
    catch (const std::exception &e)
    {
      std::cout << "警告: 无法解析位值 '" << tokens[i] << "' 在位置 " << i << std::endl;
    }
  }

  if (binary_bits.empty())
  {
    std::cout << "错误: 没有提取到有效的二进制位" << std::endl;
    return cv::Mat();
  }

  // 🎯 **确保位数正确**
  if (static_cast<int>(binary_bits.size()) < descriptor_bits)
  {
    std::cout << "警告: 实际位数 " << binary_bits.size() << " 少于期望的 " << descriptor_bits << " 位，补零" << std::endl;
    while (static_cast<int>(binary_bits.size()) < descriptor_bits)
    {
      binary_bits.push_back(0);
    }
  }
  else if (static_cast<int>(binary_bits.size()) > descriptor_bits)
  {
    std::cout << "警告: 实际位数 " << binary_bits.size() << " 多于期望的 " << descriptor_bits << " 位，截断" << std::endl;
    binary_bits.resize(descriptor_bits);
  }

  // 🎯 **转换为字节数组**
  int bytes_needed = (descriptor_bits + 7) / 8;
  std::vector<uchar> bytes(bytes_needed, 0);

  for (int i = 0; i < static_cast<int>(binary_bits.size()); ++i)
  {
    if (binary_bits[i] == 1)
    {
      int byte_idx = i / 8;
      int bit_idx = 7 - (i % 8); // 高位在前
  ...
  // 🎯 **创建OpenCV Mat**
  cv::Mat descriptor(1, bytes_needed, CV_8U);
...
// 🔧 **通用二进制描述符解析器**
cv::Mat Vocabulary::parse_generic_binary_descriptor(const std::vector<std::string> &tokens)
{
  std::cout << "尝试通用二进制解析..." << std::endl;
  ...
  // 使用实际位数
  int descriptor_bits = static_cast<int>(binary_bits.size());
  int bytes_needed = (descriptor_bits + 7) / 8;
  std::vector<uchar> bytes(bytes_needed, 0);

  for (int i = 0; i < descriptor_bits; ++i)
  {
    if (binary_bits[i] == 1)
    {
      int byte_idx = i / 8;
...
// 🔧 **逗号分隔数值解析**
cv::Mat Vocabulary::parse_comma_separated_descriptor(const std::string &desc_str)
{
  ...
  while (std::getline(ss, token, ','))
  {
    ...
    if (!token.empty())
    {
      ...
      {
        std::cout << "警告: 无法解析数值 '" << token << "'" << std::endl;
      }
    }
  }
...
cv::Mat Vocabulary::parse_binary_string_descriptor(const std::string &desc_str)
{
  ...
  int bits = static_cast<int>(desc_str.length());
...
// 🔧 **十六进制格式解析**
cv::Mat Vocabulary::parse_hex_descriptor(const std::string &desc_str)
{
  if (desc_str.empty() || desc_str.length() % 2 != 0)
  {
    return cv::Mat();
  }

  int bytes_count = static_cast<int>(desc_str.length()) / 2;
  std::vector<uchar> bytes(bytes_count);

  for (int i = 0; i < bytes_count; ++i)
  {
    std::string hex_byte = desc_str.substr(i * 2, 2);
    try
    {
      bytes[i] = static_cast<uchar>(std::stoi(hex_byte, nullptr, 16));
    }
    catch (const std::exception &)
    {
      std::cout << "警告: 无法解析十六进制字节 '" << hex_byte << "'" << std::endl;
      return cv::Mat();
    }
  }

  cv::Mat descriptor(1, bytes_count, CV_8U);
  for (int i = 0; i < bytes_count; ++i)
  {
    descriptor.at<uchar>(0, i) = bytes[i];
  }
...
// 🔧 **空格分隔数值解析**
cv::Mat Vocabulary::parse_space_separated_numbers(const std::string &desc_str)
{
  std::vector<float> values;
  std::stringstream ss(desc_str);
  std::string token;
                                                 ...
                                                 const std::vector<WordMapping> &word_mappings)
{
  ...
  // 🚀 **流式处理：分批分配内存**
  m_nodes.clear();
  m_nodes.reserve(static_cast<size_t>(max_node_id + 1)); // 预留空间，避免重复分配
  m_nodes.resize(static_cast<size_t>(max_node_id + 1));
  ...
  for (int i = 0; i <= max_node_id; ++i)
  {
    ...
    m_nodes[idx].children.clear();
    m_nodes[idx].weight = 0.0;
    m_nodes[idx].word_id = -1;
    m_nodes[idx].descriptor = cv::Mat();
  }
  ...
  for (const auto &temp_node : temp_nodes)
  {
    if (temp_node.id >= 0 && static_cast<size_t>(temp_node.id) < m_nodes.size())
    {
      size_t idx = static_cast<size_t>(temp_node.id);
      m_nodes[idx].id = temp_node.id;
      m_nodes[idx].parent = temp_node.parent;
      m_nodes[idx].weight = temp_node.weight;

      if (!temp_node.descriptor.empty())
      {
        m_nodes[idx].descriptor = temp_node.descriptor.clone();
      }
      valid_nodes++;

      if (valid_nodes % 5000 == 0)
      {
        std::cout << "已处理 " << valid_nodes << " 个有效节点..." << std::endl;
      }
    }
  }
...
void Vocabulary::setup_word_mappings(const std::vector<WordMapping> &word_mappings)
{
  ...
  for (const auto &mapping : word_mappings)
  {
    ...
    if (word_id >= 0 && word_id < static_cast<int>(m_words.size()) &&
        node_id >= 0 && node_id < static_cast<int>(m_nodes.size()))
    {
