...
public:
  ...
  // 新增：直接从YAML文本读取（避开FileStorage）
  void load_from_yaml_text(const std::string &filename);
  void load_large_file_streaming(const std::string &filename);
  void rebuild_node_relationships();
...
private:
  ...
  struct WordMapping
  {
    ...
    WordMapping() = default;
    WordMapping(int wid, int nid) : word_id(wid), node_id(nid) {}
  };



  // 新增的描述符解析方法
  cv::Mat parse_dbow3_binary_descriptor(const std::string &desc_str);
  cv::Mat parse_generic_descriptor(const std::string &desc_str);
  cv::Mat convert_binary_string_to_mat(const std::string &binary_str, int expected_bits);


  // 主分发器
  cv::Mat parse_descriptor_string(const std::string &desc_str);

  // 各种格式解析器
  cv::Mat parse_space_separated_descriptor(const std::string &desc_str);
  cv::Mat parse_comma_separated_descriptor(const std::string &desc_str);
  cv::Mat parse_binary_string_descriptor(const std::string &desc_str);
  cv::Mat parse_hex_descriptor(const std::string &desc_str);
  cv::Mat parse_space_separated_numbers(const std::string &desc_str);
  cv::Mat parse_float_array_descriptor(const std::string &desc_str);
  cv::Mat parse_generic_binary_descriptor(const std::vector<std::string> &tokens);

  // 词典构建方法
  void build_vocabulary_from_temp_data(const std::vector<TempNodeData> &temp_nodes,
                                       const std::vector<WordMapping> &word_mappings);
  void setup_word_mappings(const std::vector<WordMapping> &word_mappings);
  void validate_vocabulary_structure();

  // 辅助方法
  std::string trim(const std::string &str);
  std::string extract_value(const std::string &line);
  std::string extract_descriptor(const std::string &line);
};
