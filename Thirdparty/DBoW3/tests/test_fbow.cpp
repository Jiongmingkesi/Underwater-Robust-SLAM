...
#ifdef USE_CONTRIB
// optional nonfree xfeatures2d include removed; handled via HAVE_OPENCV_XFEATURES2D guard
...
#endif
#ifdef USE_CONTRIB
    else if(descriptor=="surf" )  #ifdef HAVE_OPENCV_XFEATURES2D
    fdetector = cv::xfeatures2d::SURF::create(400, 4, 2, false);
#else
    std::cerr << "Warning: SURF not available in this OpenCV build. Falling back to ORB." << std::endl;
