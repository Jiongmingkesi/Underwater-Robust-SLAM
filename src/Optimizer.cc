...
namespace ORB_SLAM3
{
    // Default values are overridden via Tracking::newParameterLoader(Settings*).
    float Optimizer::weightVision_ = 1.0f;
    float Optimizer::weightIMU_ = 1.0f;
            ...
            if (pMP)
            {
                ...
                if (!pFrame->mpCamera2)
                {
                    ...
                    if (pFrame->mvuRight[i] < 0)
                    {
                        ...
                        e->setInformation(Eigen::Matrix2d::Identity() * invSigma2 * weightVision_);
                    ...
                    else 
                    {
                        ...
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                        Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() * invSigma2 * weightVision_;
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (pMP)
            {
                ...
                else if (!bRight)
                {
                    ...
                    const float &invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave] / unc2;
                    e->setInformation(Eigen::Matrix3d::Identity() * invSigma2 * weightVision_);
    ...
    // Scale IMU information matrix by YAML-configurable multiplier.
    if (weightIMU_ != 1.0f)
        ei->setInformation(ei->information() * weightIMU_);
    ...
    egr->setInformation(InfoG * weightIMU_);
    ...
    ear->setInformation(InfoA * weightIMU_);
    ...
    ORB_SLAM3::VertexPose *vPose = static_cast<ORB_SLAM3::VertexPose *>(optimizer.vertex(0));

    Eigen::Matrix3d Rwb_opt = vPose->estimate().Rwb;

    
    if (std::isnan(Rwb_opt.sum()) || std::abs(Rwb_opt.determinant() - 1.0) > 1e-3)
    {
        Verbose::PrintMess("CRITICAL: IMU Optimization Exploded! Numerical error. Skipping update.", Verbose::VERBOSITY_NORMAL);
        return 0; 
    }
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    if (!pFp->mpcpi)
    {
        ei->setInformation(ei->information() * 1e-4 * weightIMU_); 
    }
    else if (weightIMU_ != 1.0f)
    {
        // With prior constraint, still apply YAML-configurable IMU weight.
        ei->setInformation(ei->information() * weightIMU_);
    }
    optimizer.addEdge(ei);
    ...
    int &num_MPs, int &num_edges, bool bLarge, bool bRecInit)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
        {
            ...
            if (weightIMU_ != 1.0f)
                vei[i]->setInformation(vei[i]->information() * weightIMU_);
            ...
            vegr[i]->setInformation(InfoG * weightIMU_);
            ...
            vear[i]->setInformation(InfoA * weightIMU_);
