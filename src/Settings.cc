...
Settings::Settings(const std::string &configFile, const int &sensor) : bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false), weightVision_(1.0f), weightIMU_(1.0f)
{
    sensor_ = sensor;
...
{
    bool foundThFarPoints;
    bool foundV;
    bool foundI;

    thFarPoints_ = readParameter<float>(fSettings, "System.thFarPoints", foundThFarPoints, false);

    float wV = readParameter<float>(fSettings, "WeightVision", foundV, false);
    if(foundV)
        weightVision_ = wV;

    float wI = readParameter<float>(fSettings, "WeightIMU", foundI, false);
    if(foundI)
        weightIMU_ = wI;
}
