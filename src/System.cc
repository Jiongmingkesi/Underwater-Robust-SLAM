...
// Sophus::SE3f System::GetTrackVelocity()
...
vector<pair<double, Sophus::SE3f>> System::GetRecentOptimizedTrajectory(int n_frames)
{
    vector<pair<double, Sophus::SE3f>> vTrajectory;
    vTrajectory.reserve(n_frames);

    
    
    
    auto it_rel = mpTracker->mlRelativeFramePoses.rbegin();
    auto it_ref = mpTracker->mlpReferences.rbegin();
    auto it_time = mpTracker->mlFrameTimes.rbegin();
    auto it_lost = mpTracker->mlbLost.rbegin();
    ...
    while (count < n_frames && it_rel != mpTracker->mlRelativeFramePoses.rend())
    {
        ...
        KeyFrame *pKF = *it_ref;
        if (pKF && !pKF->isBad() && pKF->GetMap() == pCurrentMap)
        {
            
            

            Sophus::SE3f Trw = Sophus::SE3f(); // Identity

            
            KeyFrame *pKF_curr = pKF;
            while (pKF_curr->isBad())
            {
                Trw = Trw * pKF_curr->mTcp;
                pKF_curr = pKF_curr->GetParent();
            }
            ...
            Sophus::SE3f Tcw_ref = pKF_curr->GetPose();

            
            Sophus::SE3f Tcw_curr = (*it_rel) * Trw * Tcw_ref;

            
            Sophus::SE3f Twc_curr = Tcw_curr.inverse();

            
            // Twb = Twc * Tcb
            Sophus::SE3f Twb_curr = Twc_curr * settings_->Tbc().inverse();

            
            vTrajectory.push_back(make_pair(*it_time, Twb_curr));
            count++;
        }
    ...
    std::reverse(vTrajectory.begin(), vTrajectory.end());
