...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    // YAML-configurable optimizer re-weighting multipliers.
    Optimizer::setWeightMultipliers(settings->weightVision(), settings->weightIMU());
        ...
        if(pCurrentMap->isImuInitialized())
        {
            if(bOK)
            {
                ...
                if(mCurrentFrame.mnId==(mnLastRelocFrameId+mnFramesToResetIMU))
                {
                    ...
                    Eigen::Matrix3f Rwb = mCurrentFrame.GetImuRotation();
                    Eigen::Vector3f twb = mCurrentFrame.GetImuPosition();
                    Eigen::Vector3f Vw_zero = Eigen::Vector3f::Zero(); 

                    
                    mCurrentFrame.SetImuPoseVelocity(Rwb, twb, Vw_zero);

                    
                    if (mpLastKeyFrame)
                        mCurrentFrame.mImuBias = mpLastKeyFrame->GetImuBias();
                    else
                        mCurrentFrame.mImuBias = ORB_SLAM3::IMU::Bias();
...
void Tracking::StereoInitialization()
{
    ...
    if (mCurrentFrame.N > 250)
    {
        if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
            ...
            if (!mFastInit && (mCurrentFrame.mpImuPreintegratedFrame->avgA - mLastFrame.mpImuPreintegratedFrame->avgA).norm() < 0.2)  
            {
...
// Sophus::SE3f Tracking::GetVelocity()
