...
namespace ORB_SLAM3
{
    ...
    SPmatcher::SPmatcher(float thre)
    {
        ...
        cfg.lightgluePath = "TensorRT_model/superpoint_lightglue_fp16_dynamic.engine";
    ...
    int SPmatcher::MatchingPoints_onnx(std::vector<cv::Point2f> kpts0, std::vector<cv::Point2f> kpts1, float *desc0, float *desc1)
    {
        ...
        featureMatcher->Matcher_Inference(normal_kpts0, normal_kpts1, desc0, desc1);

        
        std::vector<int> vnMatches12;
        vnMatches12 = std::vector<int>(normal_kpts0.size(), -1);
        int size = featureMatcher->Matcher_PostProcess_fused(kpts0, kpts1, vnMatches12);
    ...
    {
        vnMatches12.assign(kpts0.size(), -1);
        int rows = 500;
        int cols = 800;
        ...
        float *d0 = desc0.ptr<float>();
        float *d1 = desc1.ptr<float>();

        featureMatcher->Matcher_Inference(normal_kpts0, normal_kpts1, d0, d1);
        int size = featureMatcher->Matcher_PostProcess_fused(kpts0, kpts1, vnMatches12);
        return size;
    }
    ...
    int SPmatcher::MatchingPoints_onnx(std::vector<cv::KeyPoint> kpts0, const std::vector<cv::KeyPoint> kpts1, cv::Mat desc0, const cv::Mat desc1, std::vector<int> &vnMatches12)
    {
        ...
        for (auto &kp : kpts0)
            pts0.push_back(kp.pt);
        for (auto &kp : kpts1)
            pts1.push_back(kp.pt);

        auto normal_kpts0 = featureMatcher->Matcher_PreProcess(pts0, rows, cols);
        auto normal_kpts1 = featureMatcher->Matcher_PreProcess(pts1, rows, cols);

        float *d0 = desc0.ptr<float>();
        float *d1 = (float *)desc1.ptr<float>(); // const cast

        featureMatcher->Matcher_Inference(normal_kpts0, normal_kpts1, d0, d1);
        int size = featureMatcher->Matcher_PostProcess_fused(pts0, pts1, vnMatches12);
        return size;
    }

    int SPmatcher::MatchingPoints_onnx(Frame &f1, Frame &f2, vector<int> &vnMatches12)
    {
        vnMatches12.assign(f1.mvKeys.size(), -1);
        ...
        float *d1 = f1.mDescriptors.ptr<float>();
        float *d2 = f2.mDescriptors.ptr<float>();

        featureMatcher->Matcher_Inference(normal_kpts1, normal_kpts2, d1, d2);
        int size = featureMatcher->Matcher_PostProcess_fused(kpts1, kpts2, vnMatches12);

        

        std::cout << "[LightGlue] Frame " << f1.mnId << " -> " << f2.mnId

                  << ": matched keypoints = " << size

                  << " / " << kpts1.size() << " (ref)" << std::endl;
    ...
    int SPmatcher::SearchByBoW(KeyFrame *pKF1, Frame &F, std::vector<MapPoint *> &vpMapPointMatches)
    {
    ...
    int SPmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches12, vector<cv::DMatch> &vmatches)
    {
